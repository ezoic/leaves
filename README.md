...
Production CPU profiles (e.g. adserver CTR ensemble) spend most time in tree walk and categorical bitset tests. `lgTree.predict` copies slice headers for `nodes`, `leafValues`, `catThresholds`, and `catBoundaries` to locals once per prediction so the traversal loop does not reload those headers through the receiver on every depth level. Large categorical bitsets use `lgFindInBitset(th, bd, ...)` with those hoisted slices so each split does not re-copy `catThresholds` / `catBoundaries` from the receiver. LightGBM loaders validate categorical bitset boundaries once during model load so the prediction hot path can stay guard-free. `predict` branches categorical vs numerical decisions in the traversal loop to avoid an extra call per depth level. For categorical bitsets, `categoricalDecision` inlines the single-word (`span == 1`) test before calling `lgFindInBitset` for multi-word run-length encodings. `lgFindInBitset` rejects out-of-range word indices (`pos >> 5 >= span`) in one branch, then loads `th[idxS+word]` with a preceding touch of that same computed word index so bounds-check elimination can avoid a second check on the final load without constructing a slice header on each call. `BenchmarkLGTreeFindInBitset` covers the single-word, multi-word, and out-of-range paths; compiler BCE can be spot-checked with `go test -gcflags='-d=ssa/check_bce'`.
