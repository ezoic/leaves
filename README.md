...
[![GoDoc](https://godoc.org/github.com/ezoic/leaves?status.png)](https://godoc.org/github.com/ezoic/leaves)
...
[![Go Report Card](https://goreportcard.com/badge/github.com/ezoic/leaves)](https://goreportcard.com/report/github.com/ezoic/leaves)
...
```sh
go get github.com/ezoic/leaves
```
...
Documentation is hosted on godoc ([link](https://godoc.org/github.com/ezoic/leaves)). Documentation contains complex usage examples and full API reference. Some additional information about usage examples can be found in [leaves_test.go](leaves_test.go).
...
Production CPU profiles (e.g. adserver CTR ensemble) spend most time in tree walk and categorical bitset tests. `lgTree.predict` copies slice headers for `nodes`, `leafValues`, and `catThresholds` to locals once per prediction so the traversal loop does not reload those headers through the receiver on every depth level. Large categorical bitsets (catType 0) resolve their `catThresholds` word run at load time: `packCatRun` packs the run's start index and span into the node `Threshold` bits (like the `catMedium` inline bitset), so `categoricalDecision` tests a category with a single `catThresholds` load instead of two extra dependent `catBoundaries` loads per split — `catBoundaries` no longer exists on `lgTree` at all. The same `pos >> 5 >= span` branch covers single-word thresholds (most common sparse splits), multi-word runs, and out-of-range categories. LightGBM loaders validate every packed run against `catThresholds` once during model load (`validateCatBitsets`) so the prediction hot path can stay guard-free. After all trees load, `compactCatThresholds` merges every tree's large-bitset runs into one deduplicated `catThresholds` array shared by all trees (identical runs — common across a GBDT's trees — are stored once) and rewrites each node's `packCatRun` offset to point into it. This shrinks the memory `categoricalDecision`'s hot `catThresholds` load walks and improves CPU cache residency; because each node still reads the same bits via the same encoding, predictions are unchanged. `predict` branches categorical vs numerical decisions in the traversal loop to avoid an extra call per depth level. `TestLGTreeCategoricalDecisionPackedRun` covers hit/miss/out-of-range/negative/NaN cases; `TestCompactCatThresholds` proves dedup preserves every decision and shares one backing array; `BenchmarkLGTreeCategoricalDecisionSingleWord` and `BenchmarkLGTreeCategoricalDecisionMultiWord` track the hot path.

## Development checks

Run `GOWORK=off go test -short ./...` and `GOWORK=off go vet ./...` before release-tagging changes. Transformation struct literals use keyed fields so `go vet` can catch API-drift risks across the package boundary.
